...
for (var in categorical_vars) {
  ggplot(dat, aes(x = !!as.name(var))) +
    geom_bar() +
    labs(x = var, y = "Count") +
    ggtitle(paste("Bar Graph of", var))
}
...
# Generate a sequence of values to represent the range of observed data
x <- seq(min(actual_productivity), max(actual_productivity), length.out = 100)

# Compute the probability density function (PDF) of the optimal distribution
pdf_optimal_distribution <- dnorm(x, mean = optimal_distribution_fit$estimate[1], sd = optimal_distribution_fit$estimate[2])


# Plot the histogram of actual data and the fitted distribution
hist(actual_productivity, freq = FALSE, main = "Comparison of Actual Data and Fitted Distribution", xlab = "Productivity Score")
...
# Grouping by quarter, department, and day, and summarizing descriptive statistics for target and actual productivity scores
grouped_data <- dat %>%
  group_by(quarter, department, day) %>%
  summarize(
    # 평균 값 도출.
    mean_target_score = mean(targeted_productivity),
    mean_actual_score = mean(actual_productivity),
    # 중앙값 도출.
